...
void publishStoredMessages() {
  ...
  // Serial.println(data);
 mqttClient.publish("/test/datetime", message.c_str());
...
//void publishStoredMessages() {
//  File inFile = SPIFFS.open("/mqtt_messages.txt");
//  if (!inFile) {
//    Serial.println("Failed to open file for reading");
//    return;
//  }
//
//  File outFile = SPIFFS.open("/mqtt_messages_temp.txt", FILE_WRITE);
//  if (!outFile) {
//    Serial.println("Failed to create temp file for writing");
//    inFile.close();
//    return;
//  }
//
...
//    String message = inFile.readStringUntil('\n');
//    message.trim(); // Remove leading/trailing spaces and newlines
//
...
//      // delay(100);
//    } else {
//      // If the message is empty, don't publish it and skip writing to the temp file
//      continue;
//    }
//
//    // Write the message back to the temp file if it was not published
...
//  SPIFFS.remove("/mqtt_messages.txt");
//  SPIFFS.rename("/mqtt_messages_temp.txt", "/mqtt_messages.txt");
//}
...
long period=lastmillis2-lastmillis1;
unsigned long myperiod;
if (period>=0){myperiod=period;}
else{myperiod=period*(-1);}
...
//sending the mean value via MQTT
 ...
 sprintf(data, "%i", meanvalue);
  // Serial.println(data);
     mqttClient.publish("/swa/mean", data);
...
//because 
 ...
 sprintf(data, "%i", lp);
   //Serial.println(data);
     mqttClient.publish("/swa/lp1", data);

  sprintf(data, "%i", e);
  // Serial.println(data);
     mqttClient.publish("/swa/nb", data);

 String per= String(myperiod)+" ms";
 
 sprintf(data, "%i", per);
  // Serial.println(data);
     mqttClient.publish("/swa/period", data);
